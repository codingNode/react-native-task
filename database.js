...
export async function createTable() {
  return new Promise((resolve, reject) => {
   return db.transaction(
      (tx) => {
        tx.executeSql(
          'create table if not exists menuitems (id integer primary key autoincrement not null, uuid text, title text, price text, category text);',[]
        );
      },
      reject,
      resolve
...
export function saveMenuItems(menuItems) {
  ...
  db.transaction((tx) => {
    ...
    menuItems.menu.map((item)=>{
       tx.executeSql(
      `insert into menuitems(uuid,title,price,category) values (?,?,?,?)`, [item.id,item.title,item.price,item.category.title],(txObj,result)=>{console.log("inserted: ", result.rows)},(txObj,error)=>console.log("Error from the database", error)
      )
    })

  });
  // db.transaction((tx)=>{
  //   tx.executeSql('select * from menuitems',null,(txObj,result)=>{console.log("reading: ", result.rows._array)},(txObj,e)=>{console.log("reading error: ",error)})
  // })
}
...
let sql='select * from menuitems where category=?'

  const promises = activeCategories.map((item)=>{
    return new Promise((resolve, reject) => {
      db.transaction((tx)=>{
        // query execution
...
if(query)
{
  let q_result= await queryData(query);
  ...
  try {
    ...
    const commonData = newData.filter(el => q_result.map(item=>item.title).some(title=>title === el.title));
...
async function queryData(query)
{
  ...
  const query_promise = new Promise((resolve,reject)=>{
       db.transaction((tx)=>{
         tx.executeSql(sql,['%' + query + '%'],(txObj, result)=>{
           resolve(result.rows._array)
         },(txObj,e)=>{console.log(e); reject(e)})
       },null,null)
  });
  try {
 
   const query_result = await Promise.resolve(query_promise);
   return query_result;   
 } catch (error) {
   console.error(error);
 }
